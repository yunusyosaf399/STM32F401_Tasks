...
#define FLASH_SECTOR_ADDRESS  0x08020000  // Example: Start of Sector 5
...
int main(void)
{
	HAL_Init();
	    ...
	    // Read the value stored at the flash address
	    uint32_t readData = Flash_Read(FLASH_SECTOR_ADDRESS);

	    // Convert the 32-bit readData into a hex string for UART transmission
	    char dataString[9];  // 8 characters for hex + null terminator
	    sprintf(dataString, "0x%08X\r\n", readData);  // Format as "0x12345678"

	    /* Infinite loop */
	    while (1)
	    {
	        // Transmit the hex string over UART
